	...
	std::string &str_number, const int &precision)
{
	...
	if (dot_index == std::string::npos) {
		...
		// Add decimal separator and zeros if number precision is less than given precision.
		if (precision > 0) {
			str_number += ".";
		...
		if (!string_utility::is_numbers_only(str_number.substr(0, dot_index))
			|| !string_utility::is_numbers_only(str_number.substr(dot_index + 1)))
		{
			throw std::invalid_argument("string is not a valid number.");
		}

		if (str_number.size() > INT_MAX) {
			throw std::overflow_error("string size should be less than INT_MAX.");
		}
		...
		if (deficit_digit_count > 0) {
			...
			// Start rounding from smallest digit, right to left until precision point.
			for (size_t i = str_number.size() - 1; i > dot_index + precision; i--) {
				if (str_number[i] - '0' >= 5) {
					if (i == dot_index + 1) {
						str_number[i - 2] += 1;
					}
